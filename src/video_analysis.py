...
class VideoAnalyzer:
    """
    Analysis of a given video is done using a class so we can better keep track of the information in the video
    """
    def __init__(self, video_dir:str, output_dir:str, id:str):
        self.video_dir = video_dir
        self.output_dir = output_dir
        os.makedirs(self.output_dir, exist_ok=True) # Make sure output dir exists
        self.id = id
        # analyze video
    ...
    def run_detection(self, model:str, acc=3):
        """
        ...
        # prepare output mp4
        out_mp4 = cv2.VideoWriter(f'{self.output_dir}/{self.id}_full_out.mp4',
            ...
            if success:
                ...
                if result.boxes and result.boxes.is_track:
                    ...
                    for i in range(0, len(boxes)):
                        ...
                        track_t[round(time_index / self.fps, acc)] \
                            = str(round(float(x),acc)) + ", " + str(round(float(y),acc))
        ...
        output_filename = f'{self.output_dir}/{self.id}_output_data.csv'
        df.to_csv(output_filename)

        return (df, results)


    def specific_output(self, requested_ids:list, results):
        """
        ...
        name = f"{self.output_dir}/{self.id}_collision_{str(requested_ids[0])}_{str(requested_ids[1])}.mp4"
        out_mp4 = cv2.VideoWriter(name, cv2.VideoWriter_fourcc(*"XVID"),
                                  self.fps, self.resolution)
