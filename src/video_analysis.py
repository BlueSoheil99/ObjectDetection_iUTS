...
def video_analysis(video_path : str, id=0, acc=3):
    ...
    # prepare output mp4
    out_mp4 = cv2.VideoWriter("data/output/"+str(id)+"_full_out.mp4", cv2.VideoWriter_fourcc(*"XVID"), fps, resolution)
    ...
    track_data = defaultdict(lambda: defaultdict(lambda: "")) # data is stored in this format {track_id:(time_index, x, y), ...}
        ...
        if success:
            ...
            if result.boxes and result.boxes.is_track:
                ...
                for i in range(0, len(boxes)):
                    ...
                    track_t[round(time_index / fps, acc)] = str(round(float(x),acc)) + ", " + str(round(float(y),acc)) # change acc input for more specific timings
    ...
    output_filename = "data/output/"+str(id)+"_output_data.csv"
    df.to_csv(output_filename)

    return (df, results, (fps, resolution))

# results is a results object from ultralytics
def specific_output(requested_ids : list, results, fps_resolution : list, id=0):
    ...
    name = "data/output/"+str(id)+"_collision_" + str(requested_ids[0]) + "_" + str(requested_ids[1]) + ".mp4"
